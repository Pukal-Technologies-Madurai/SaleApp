...
const DeliveryTable = ({ deliveryData }) => {
    ...
    const filteredData = useMemo(() => {
        ...
        // Sort by Delivery_Time ascending; fallback to Created_on.
        // New/Nil rows are always pushed to the bottom.
        // Remaining rows use Delivery_Time, then Created_on as fallback.
        return filtered.sort((a, b) => {
            ...
            const isNewOrNil = item =>
                item?.Delivery_Status === 0 || item?.Delivery_Status === 1;

            const aIsNewOrNil = isNewOrNil(a);
            const bIsNewOrNil = isNewOrNil(b);

            if (aIsNewOrNil && !bIsNewOrNil) return 1;
            if (!aIsNewOrNil && bIsNewOrNil) return -1;

            const timeA = getTimeValue(a.Delivery_Time);
            const timeB = getTimeValue(b.Delivery_Time);
            const createdA = getTimeValue(a.Created_on);
            const createdB = getTimeValue(b.Created_on);

            const sortA = timeA ?? createdA;
            const sortB = timeB ?? createdB;
    ...
    const formatTimeDummy = dateString => {
    ...
    const getStatusDisplay = item => {
        if (item.Delivery_Status === 7 || item.Delivery_Status === 6) {
            return formatTime(item.Delivery_Time) || formatTimeDummy(item.Created_on) || "Done";
        }
